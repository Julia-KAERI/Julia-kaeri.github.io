...
- 기계학습에서의 학습이란 대량의 데이터를 입력하여 이 데이터를 가장 잘 표현하는 하나의 함수를 정하는 것이다. 하나의 데이터에 대해 보통 입력값이 여러개이므로  $i$-번째 데이터의 입력은 $\bf{x}_i$ 로 $i$-번째 데이터의 label 은 $y_i$ 로 표기한다. 입력값과 label 의 쌍을 $(\bf{x}_i, y_i)$  로 표기한다. 
...
- 함수가 얼마나 데이터를 잘 기술하는지를 평가하는 함수를 **손실함수(Loss function)** 혹은 **비용함수(Cost function)** 라고 한다. 대표적인 손실함수로는 신경망(Neural Network) 에서, 회귀의 경우 평균제곱오차 (Mean Square Error, MSE) 함수 와 이진 분류의 경우 교차 엔트로피 오차 (Cross Entropy Error, CEE) 함수가 있다.

$$
\begin{aligned}
\text{MSE}(\bf{w}) &= \dfrac{1}{2} \sum_{i=1}^N \left(y_i - f(\bf{x}_i; \bf{w})\right)^2 \\[0.3em]
\text{CEE}(\bf{w}) &= \sum_{i=1}^N \left[y_i \ln (f(\bf{x}_i;\bf{w})) - (1-y_i)(1-\ln (f(\bf{x}_i;\bf{w})))\right]
\end{aligned}
$$
