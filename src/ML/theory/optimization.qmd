...
다음과 같은 최적화 문제가 주어졌다고 하자. 일반적으로 최적화 문제는 다음과 같이 주어진다.
...
1. 목적함수가 convex function 이고 가능해 영역 이 convex set 인 경우의 최적화 문제를 **볼록 최적화 (convex optimization)** 문제 라고 한다.
2. $g_i(\bf{x})$ 가 convex function 이고 $h_j(\bf{x})=\bf{a}^T\bf{x}+b$ 꼴이면 항상 볼록 최적화 문제이다.
3. Convex optimization 의 모든 국소적 해는 전역적 해이다. (@thm-optimization_convex_local_global_minima)
