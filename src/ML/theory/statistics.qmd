...
$N$ 개의 입력 변수 $\bf{x} = \begin{bmatrix} x_1 &\ldots & x_N \end{bmatrix}^T$ 와 표적값 $\bf{t}=\begin{bmatrix} t_1 &\ldots &t_N\end{bmatrix}^T$ 사이에 polynomial $t=y(x;\bf{w})=w_0 + w_1x+ \cdots +w_nx^n$ 의 관계를 가정한다. 표적값의 불확도를 확률분포로서 표현하자. 이를 위해 주어진 $x$ 에 대해 표적값의 확률은 $y(x,\,\bf{w})$ 를 중심으로 분산이 $\beta^{-1}$ 인 가우시안분포를 따른다고 가정한다. 즉,
...
이 된다. 여기서 (고정된 $\beta$ 에 대해) $p(\bf{t}\mid \bf{x},\, \bf{w},\,\beta)$ 를 최소화 하는 것과 $\displaystyle \dfrac{1}{2}\sum_{n=1}^N \left[ y(x_n,\,\bf{w})-t_n\right]^2$ 를 최소화하는 것은  동치이며 이렇게 하는 $\bf{w}$ 를 $\bf{w}_{ML}$이라 하자. 또한 $\beta$ 에 대해 미분하여 $p$ 를 maximize 하는 $\beta$ 를 찾아 $\beta_{ML}$ 이라 하면,
$$
\dfrac{1}{\beta_{ML}}=\dfrac{1}{N}\sum_{n=1}^N \left[ y(x_n,\, \bf{w})-t_n\right]^2 
...
- 앞서 우리는 prior distribution $p(\bf{w}|\alpha)$ 에 대한 추정을 포함시켰지만, $\bf{w}$ 에 대한 point estimate 이므로 이것은 제대로 된 Bayesian treatment 가 아니다. 제대로 된 베이지언에서는 확률에 대한 합과 곱의 규칙들을 일관되게 적용해야 하며, 이는 $\bf{w}$ 에 대한 모든 값에 대해 적분해야 함을 의미한다. 이러한 marginalizations 가 패턴턴 인식에서의 베이지언 방법의 핵심이다.
