...
- 최소자승법을 이용한 다항함수 피팅 에서 보았듯이 best generalization을 주는 최적의 다항식의 order $M$ 이 존재한다. 다항식의 order는 모델에서 free parameters의 갯수를 제어한다. Regularization 을 사용하면 regularization coefficient $\lambda$ 는 모델의 유효 복잡도(effiective complexcity) 를 통제한다. 
