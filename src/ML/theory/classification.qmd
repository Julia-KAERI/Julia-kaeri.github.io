...
두개의 클라스 $C_1,\,C_2$ 로 분류하는 문제를 살펴 보자. @eq-ML_classification_generalized_linear_model 에서의 활성화 함수 $f$ 를 $\text{sign}(a)$ 함수 즉, 
...
$f(\bf{x};\,\bf{\theta})$ 를 통해 데이터 셋 $\{(\bf{x}^{(i)},\, y^{(i)}):i=1,\ldots,\,N\}$ 을 기술한다고 하자. 이 때 비용함수로 가장 많이 사용되는 것은 교차 엔트로피 
