...
이 문제를 해결하기 위해 확률론을 도입한다. 우선 퍼셉트론의 출력 함수를 시그모이드 함수

$$
\sigma(t) = \dfrac{1}{1+e^{-t}}
$$

로 정의한다. 시그모이드 함수 $\sigma (t)$ 의 치역은 $[0,\,1]$ 사이에 있으며 

$$
y=\sigma\left(\bf{w}^T\bf{\phi}\left(\bf{x}\right)\right)
$$

를 $\bf{y}$ 가 어떤 클래스에 속할 확률로 생각한다. 
...
그리고 이에 대한 비용 함수는 이진 교차 엔트로피(Binary cross entropy, BCE) 함수를 사용한다. $\hat{y}^{(i)} = \sigma \left(\bf{w}^T\bf{\phi}\left(\bf{x}^{(i)}\right)\right)$ 에 대해
...
이렇게 이진 분류 문제에 시그모이드 함수를 도입하여 확률론적으로 최적화 하는 것을 **로지스틱 회귀(logistic regression)** 이라고 한다. 회귀(regression) 이라는 말이 들어가지만 회귀가 아닌 분류 문제에 사용된다.

</br>

### 다중 클래스 로지스틱 회귀

$C_1,\ldots,\,C_K$ 클라스로 분류하는 데 사용되는 판별함수 $a_1 = a_1(\bf{x};\bf{w}_1),\ldots,\,a_K=a_K(\bf{x};\bf{w}_K)$ 를 원-핫 벡터로 하고 이 값을 기준으로 확률적으로 어떤 클래스에 속하는지 판단한다고 하자. 여기에는 소프트 맥스 변환을 이용한다. $\bf{a}\in \R^K$ 에 대한 소프트 맥스 변환 $\bf{\gamma}:\R^K \to \R^K$ 은
$$
\bf{y} = \bf{\gamma}(\bf{a})= \dfrac{1}{\sum_{j=1}^K e^{a_j}} \begin{bmatrix} e^{a_1} \\ \vdots \\ e^{a_K}\end{bmatrix}
$$

로 $\bf{y}$ 의 각 성분은 $[0,\,1]$ 범위의 값을 가지며 그 성분의 합은 $1$ 이다. 따라서 $\bf{y}$ 의 각 성분은 확률로 해석 할 수 있다. 이렇게 다중 클래스 분류 문제를 확률로 해석하는 것을 다중 클래스 로지스틱 회귀라고 한다. 


다중 클래스 로지스틱 회귀에 대한 비용함수로는 **교차 엔트로피 오차 함수(Cross entropy error function, CEE)** 가 사용된다.

$$
\text{CEE}(\bf{w}) = -\sum_{i=1}^n\sum_{j=1}^K   y^{(i)}_j \ln \left(\hat{y}^{(i)}_j\right) =  -\sum_{i=1}^n\sum_{j=1}^K  y^{(i)}_j \left[ a_j - \ln \left(\sum_{m=1}^K e^{a_m}\right) \right]
$$
