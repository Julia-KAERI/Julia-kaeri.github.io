...
이다. 여기서 $\dfrac{\partial \tilde{z}^{(M)}_i}{\partial w^{(M)}_{ij}}$ 는 순전파 과정에서 정해지므로 마지막 은닉층의 매개변수에 대한 손실함수의 편미분을 모두 구할 수 있다. 이제 $m$ 번 은닉층의 매개변수에 대한 편미분은
