...
라 할 수 있다. 이것을 독같이 세번째 은닉층에도 할 수 있다. 이제 출력층을 보자. 보통 출력층에서의 활성화 함수는 다른 은닉층의 활성화 함수와 다르기 때문에 $\sigma_O$ 라고 하자. 은닉층이 3개이므로 
...
이다. 기본적으로 출력층에서의 편미분은 자동 미분 기법에 의해 쉽게 계산 된다. 

$$
\dfrac{\partial L(\bf{\theta})}{\partial w^{(O)}_{ij}}
$$
 

$$
\dfrac{\partial L(\bf{\theta})}{\partial w^{(m)}_{ij}} = \sum_{k_{M}=1}^{N_{M}} \dfrac{\partial L(\bf{\theta})}{\partial w_{kl}^{(O)}} \dfrac{\partial }
$$
