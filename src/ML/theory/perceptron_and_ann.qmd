...
입력층 $\bf{x} = \begin{bmatrix}x_1 & \cdots & x_D\end{bmatrix}^T$ 에 대해 첫번째 은닉층의 $j$ 번째 퍼셉트론의 내부 파라미터 벡터를 $\bf{w}^{(1j)}$ 와 $b^{(1)}_j$ 로 표기하기로 하고
...
$$
\bf{h}^{(1)}= F^{(1)}\left(\bf{z}^{(1)}\right) = \begin{bmatrix} \sigma \left(z^{(1)}_1\right) \\ \vdots \\ \sigma \left({z}^{(1)}_{N_1}\right) \end{bmatrix}
$$
