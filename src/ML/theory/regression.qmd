...
$$
\bf{\theta}^\ast = \arg\min_\bf{\theta} \left\|\bf{y}- \bf{\Phi \theta}\right\|
...
$$
\bf{\theta}^\ast = \arg\min_\bf{\theta} \left\|\bf{y}- \bf{\Phi \theta}\right\|_p
...
$$
\bf{\theta}^\ast = \arg \min_\bf{\theta} \|\bf{y}-\bf{\Phi\theta}\|_2^2
$$ {#eq-linear_regression_master_equation_with_L2}

이며 $\arg \min_\bf{\theta}\|\bf{y}-\bf{\Phi\theta}\|_2^2$ 가 $\sqrt{\cdots}$ 가 없어 미분 계산이 훨씬 간단해 지기 때문에 노름의 제곱을 많이 사용한다.
 
</br>

### 학습/훈련과 예측{#sec-regression_training_and_prediction}

우리가 이미 획득한 입력 벡터의 집합 $\{\bf{x}_i\}$ 와 레이블(label) $\{y_i = y_i(\bf{x}_i)\}$ 로 이루어진 데이터 $\{(\bf{x}_i,\, y_i):i=1,\ldots,\,m\}$, 그리고 데이터를 잘 설명할것이라고 기대되는 함수 $f(\bf{x},\,\bf{\theta})$ 에 대해 @eq-linear_regression_master_equation 의 $\bf{\theta}^\ast$ 를 얻는 것을 훈련 혹은 학습 이라고 한다. 이 $\bf{\theta}^\ast$ 에 대해 $\bf{x}$ 의 상황에서 $f(\bf{x};\bf{\theta}^\ast)$ 의 값을 얻을것이라고 기대한다. 즉 우리는 주어진 데이터를 학습시켜 미지의 상황을 예측 할 수 있다. 이것은 비단 선형 회귀 뿐만 아니라 모든 통계학적 처리 및 기계학습의 과정에 공통된다.
...
$$
f(\bf{x};\bf{\theta}) = \theta_0 + \theta_1\phi_1(\bf{x}) + \cdots + \theta_m \phi_m (\bf{x})
$$

그렇다면 설계 행렬 $\bf{\Phi}$ 는

$$
\bf{\Phi} = \begin{bmatrix} 1 & \phi_1(\bf{x}_1) & \cdots & \phi_m(\bf{x}_1) \\1 & \phi_1(\bf{x}_2) & \cdots & \phi_2(\bf{x}_2) \\ \vdots & & & \vdots \\ 1 & \phi_m(\bf{x}_1) & \cdots & \phi_m(\bf{x}_1) \end{bmatrix}
$$
...
$$
\bf{\theta}^\ast =   \arg \min_\bf{\theta} \left(\|\bf{y}-\bf{\Phi \theta}\|_2^2 + \lambda \|\bf{\theta}\| \right)
