...
$n$ 차원 벡터 입력변수 $\bf{x}_1,\ldots,\,\bf{x}_m \in \R^n$ 와 입력 변수 각각에 대한 레이블 $y_1,\,\ldots,\,y_m$ 을 가장 잘 기술하는 함수
...
- 여기서 주의할 것은 $\{\bf{x}_i\}$ 에 대해 선형이기 때문에 선형 회귀가 아니라 $\bf{\theta}=  \begin{bmatrix}\theta_0 & \theta_1 & \cdots & \theta_m\end{bmatrix}$ 에 대한 선형 함수이므로 선형 회귀라는 것임.


이제 $\bf{x}_i \in \R^n$ 의 $j$ 번째 성분을 $(\bf{x}_i)_j$ 라고 표기하기로 하고 아래와 같이 행렬과 벡터를 정의한다.
