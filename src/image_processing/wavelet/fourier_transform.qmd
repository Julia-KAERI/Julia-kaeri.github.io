...
위상공간 $X$ 에서 정의된 $f:X \to \mathbb{R}$ 에 대해 $S=f^{-1}(\{0\})$ 의 closure $\overline{S}$ 를 $f$ 에 대한 support 라고 하고 $\textrm{supp}(f)$ 라고 쓴다. $\textrm{supp}(f)$ 가 옹골집합(compact set) 일 때 $f$ 를 **콤팩트 지지 함수 (compactly supported function)** 라고 한다.

:::

:::

$\mathbb{R}$ 에서의 옹골집합은 유계인 닫힌 집합과 동치이다. 즉 $\mathbb{R}$ 에서의 콤팩트 지지함수는 support 가 유계인 집합이다. 그렇다면 $f$ 가 $L^2(\mathbb{R})$ 에서의 콤팩트 지지함수라면  @thm-wavelet_norm_of_fourier_transfrom_in_L2 에 따라 푸리에 변환이 존재하며 $\|f\|_2 = \left\|\hat{f}\right\|_2$ 이다. 
...
$$
\lim_{n\to \infty}\dfrac{1}{2\pi} \int_{-n}^n e^{i\omega t}\hat{f}(\omega)\, d\omega = f(t)
...
$$
\begin{aligned}
f(t) = \overline{\hat{g}(t)} &= \lim_{n \to \infty} \dfrac{1}{2\pi} \overline{\int_{-n}^n e^{-i\omega t}\, g(\omega)\, d\omega} \\
&= \lim_{n \to \infty} \dfrac{1}{2\pi} \int_{-n}^n e^{i\omega t} \overline{g(\omega)}\, d\omega  = \lim_{n \to \infty}\dfrac{1}{2\pi} \int_{-n}^n e^{i\omega t}\, \hat{f}(\omega)\, d\omega. \qquad \square
\end{aligned}
$$
...
여기서 @eq-wavelet_inverse_fourier_transform_in_L2 를 **푸리에 역변환 (inverse Fourier transform)** 이라고 하고 $\mathfrak{F}^{-1}\left[\hat{f}\right]$ 라고도 쓴다. 푸리에 변환과 역변환을 다음과 같이 정의하면 대칭형이 된다.

$$
\hat{f}(\omega) = \dfrac{1}{\sqrt{2\pi}} \int_{-\infty}^\infty e^{-i\omega t}\, f(t)\, dt,\qquad f(t) = \dfrac{1}{\sqrt{2\pi}}\int_{-\infty}^\infty e^{i\omega t}\, \hat{f}(\omega)\, d\omega. 
$$
