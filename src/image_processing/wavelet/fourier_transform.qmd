...
$$
t_0=a < t_2 < \cdots < t_{N-1} < t_{N-1} = b.
$$

계산을 간단하게 하기 위해 동일한 거리를 갖는 $t_k = a + k\dfrac{b-a}{N-1}$ 라고 할 수 있다. $f_k: = f(t_k)$ 라고 하면 수열 $\langle f \rangle = (f_0,\ldots,\, f_{N-1})$ 를 정의 할 수 있다. 
...
#### 이산 푸리에 변환
$\langle f\rangle = (f_0,\ldots,\,f_{N-1})$ 에 대해 아래와 같이 정의된 $\mathfrak{F}[f]:=\langle F\rangle = (F_0,\ldots,\,F_{N-1})$ 을 $\langle f\rangle$ 에 대한 **이산 푸리에 변환(descrete Fourier transform)** 이라고 한다.

$$
\langle F_u \rangle := \mathfrak{F}[f]_u=\dfrac{1}{N} \sum_{k=1}^{N} f_k \exp\left(-\dfrac{i2\pi uk }{N}  \right).
$$ {#}


::: {#thm-wavelet_identity_of_dft}

$(f_k)$ 에 대한 DFT 에 ㄷ$f_k$ 는 다음과 같이 표현 할 수 있다.

$$
f_k = \mathfrak{F}^{-1}[F]_k = \sum_{u=1}^{N} F_u \exp \left(\dfrac{i2\pi uk}{N}\right)
$$
