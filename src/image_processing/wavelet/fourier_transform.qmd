...
고속 푸리에 변환(Fast Fourier Transform, FFT)은 DFT 과 그 역변환을 빠르게 수행하는 효율적인 알고리즘이다. 위의 푸리에 변환은 $\mathcal{O}(N^2)$ 의 연산이 필요하지만, FFT를 이용하면 $\mathcal{O}(N \log N)$ 의 연산만으로 가능하다. 기본적인 아이디어는 가우스가 생각했었으나 잊혀졌고 이후 Cooley 와 Tukey 가 1965년도에 고속 푸리에 변환 알고리즘을 발표하였으며 이후 전 세계적으로 광범위하게 사용되었다.

</br>

### 기본적인 아이디어

우선 $N=2^m$ 일 경우에 대해 생각하자. 그리고 우리는 $f_k$ 의 인덱스 $k$ 를 $1\le k \le N$ 에서 생각했지만 여기서는 $0\le k \le N-1$ 로 생각하자. 이 경우, 
...
이다. $w_N := e^{2\pi i /N},\, M=N/2=2^{m-1}$ 이라 하면 ${w_N}^2 = w_M = e^{2\pi i /M} = e^{2\pi i /(N/2)}$ 이며, 이를 이용하여

$$
\begin{aligned}
\hat{f}_u &= \sum_{k=0}^{N-1} f_k e^{2\pi i uk/N} =  \sum_{k=0}^{N-1}f_k {w_N}^{uk} \\
&= \sum_{k=0}^{M-1}a_k {w_N}^{2uk} + \sum_{k=0}^{M-1} b_k {w_N}^{(2k+1)u}\\
&= \sum_{k=0}^{N-1} a_k (w_M)^{uk} + \left({w_N}^u\right) \sum_{k=0}^{M-1} b_k (w_M)^{uk} \\
&= \mathfrak{F}[a]_u + \left({w_N}^u\right)\mathfrak{F}[b]_u
\end{aligned}
$$
