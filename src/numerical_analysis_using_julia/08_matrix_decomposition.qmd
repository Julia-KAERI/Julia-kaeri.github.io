...
이렇게 정해진 $\boldsymbol{v}$ 를 **하우스홀더 벡터(householder vector)** 라고 한다. 이 때 $\boldsymbol{x}$ 의 $\boldsymbol{v}$ 에 대한 하우스홀더 변환 $\boldsymbol{H}_{\boldsymbol{v}}\boldsymbol{x}$ 는 다음과 같다.
