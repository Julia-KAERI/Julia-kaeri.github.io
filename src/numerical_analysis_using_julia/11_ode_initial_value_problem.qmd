...
$$
x(t_{i+1}) = x(t_i) + f(t_i, x(t_i))h, \qquad i=0,\,1,\,2,\ldots
$$
...
&= x(t_i) + \tilde{f}(t_i, h, x) +O(h^k)
\end{aligned}
$$

을 $k>2$ 에 대해 만족하도록 할 수 있다. 이 $\tilde{f}(t_n, h, f)$ 를 얻는 방법에는 오일러 방법, 룽게-쿠타 방법 등 여러가지가 존재한다. 
