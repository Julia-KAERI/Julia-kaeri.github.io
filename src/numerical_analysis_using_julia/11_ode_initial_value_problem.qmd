...
로 풀 수 있다. 이것이 잠시 후 다를 오일러 방법이다. 우리는 앞서 [리차드슨 외삽법](06_calculus_of_one_variable_function.qmd#sec-richardson_extrapolation) 에서 보았던 것처럼 주변의 복수의 값을 이용하여 정밀도를 높일 수 있다. 즉 $F(t_i, f(t_i))$ 뿐만 아니라 몇몇 $\alpha_j$ 값에 대해 $F(t_i, f(t_i + \alpha_j h))$ 를 선형결합하여 
...
$t\in I= [a,\,b]$ 와 [$n$-cell](02_notations_and_propositions.qmd#def-n_cell) $D$ 의 데카르트곱 $I \times D$ 에서 정의되며 연속인 함수 $f(t, \boldsymbol{x})$ 가 
...
집합 $D\subset \mathbb{R}\times \mathbb{R}^{n}$ 가 모든 $\lambda \in [0,\,1]$ 와 $\boldsymbol{x}_1,\,\boldsymbol{x}_2\in D$ 에 대해 $\lambda \boldsymbol{x}_1 + (1-\lambda)\boldsymbol{x}_2\in D$ 일 때 $D$ 를 **convex** 라고 한다.
:::
:::
...
$$
\forall (t,\,\boldsymbol{x})\in D,\, \left\|\sum_{i=1}\partial_i f(t, \boldsymbol{x}) \right\| \le L
$$

이면 $f$ 는 변수 $\boldsymbol{x}$ 와 Lipschitz 상수 $L$ 에 대해 Lipschitz 조건을 만족한다.
...
::: {#thm-initial_value_problem}

$D=\{(t,\, \boldsymbol{x}) : t\in [a,\,b],\, \boldsymbol{x} \in \mathbb{R}^n\}$ 에서 정의된 함수 $\boldsymbol{f}:D\to \mathbb{R}^n$ 이 $D$ 에서 Lipsschitz 조건을 만족하면 $\boldsymbol{x} = \boldsymbol{f}(t, \boldsymbol{x})$ 의 초기값 문제는 유일한 해를 갖는다.
...
$$
x(t_{i+1}) =  x(t_i) + f(t_i, x_i) h
$$

로 근사한다. 이 때 $t_i,\,i=0,\,1,\ldots$ 를 **mesh point** 라고 한다.
...
라 하자. 그렇다면, 야코비안 행렬 $\left(\boldsymbol{J}_\boldsymbol{f}(t_i, \boldsymbol{x}(t_i))\right)_{jk} = \dfrac{\partial f_j}{\partial x_k}(t_i, \boldsymbol{x}(t_i))$ 에 대해 
