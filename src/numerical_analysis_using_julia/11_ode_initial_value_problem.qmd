...
$f(t)$ 에 대한 미분방정식이 $f$ 에 대해 최대 $n$ 차 미분까지 포함되었을 경우, $n$-차 상미분 방정식이라 한다. 일반적으로 $n$ 차 미분 방정식은 다음 꼴로 주어진다.

$$
f^{(n)}(t) = F\left(t, f(t), f'(t), \ldots, f^{(n-1)}(t)\right)
$$

이 방정식과 초기값 $f(t_0),\,f^{(1)}(t_0), \ldots, f^{(n-1)}(t_0)$ 을 통해 $f(t)$ 를 알아내는 것을 **초기값 문제 (initial value problem)** 이라고 한다. 
...
$$
f(t_{i+1}) = f(t_i) + F(t_i, f(t_i))h, \qquad i=0,\,1,\,2,\ldots
$$

로 풀 수 있다. 이것이 잠시 후 다룰 오일러 방법이다. 우리는 앞서 [리차드슨 외삽법](06_calculus_of_one_variable_function.qmd#sec-richardson_extrapolation) 에서 보았던 것처럼 주변의 복수의 값을 이용하여 정밀도를 높일 수 있다. 즉 $F(t_i, f(t_i))$ 뿐만 아니라 몇몇 $\alpha_j$ 값에 대해 $F(t_i, f(t_i + \alpha_j h))$ 를 선형결합하여 

$$
\begin{aligned}
f(t_{i+1}) &= f(t_i) + \sum_j c_j F(t_i, f(t_i + \alpha_j h))h + O(h^k), \\
&= f(t_i) + \tilde{F}(t_i, h, f) +O(h^k)
\end{aligned}
$$

이며, $k>2$ 이 되도록 할 수 있다. 이 $\tilde{F}(t_n, h, f)$ 를 얻는 방법에는 오일러 방법, 룽게-쿠타 방법 등이 존재한다. 
...
과 초기조건 $\boldsymbol{x}(t_0) ,\ldots,\,\boldsymbol{x}^{(n-1)}(t_0)$ 를 만족하는  $\boldsymbol{x}:[a,\,b] \to \mathbb{R}^m$ 을 구한다고 하자. $\boldsymbol{x} = \begin{bmatrix} x_1(t) & \cdots & x_m(t) \end{bmatrix}^T$ 라 할 때 1변수의 경우와 마찬가지로

$$
\boldsymbol{x}(t_{i+1}) =  \boldsymbol{x}(t_i) +\tilde{\boldsymbol{F}}(t_i, \boldsymbol{h}, \boldsymbol{x}, \ldots, \boldsymbol{x}^{(n-1)}) 
$$
...
집합 $D\subset \mathbb{R}^{2}$ 가 모든 $(t_1,\,x_1),\, (t_2,\,x_2)\in D$ 에 대해 
$$
\lambda (t_1,\,x_1) + (1-\lambda)(t_2,\,x_2)\in D
$$

일 때 $D$ 를 **convex** 라고 한다.
:::
:::
...
$D\subset \mathbb{R}^2$ 에서 연속인 함수 $f(t,x) : D \to \mathbb{R}$ 가 모든 $(t,x_1),\, (t,x_2)\in D$ 에 대해 어떤 $L>0$ 이 존재하여
...
::: {#prp-convex}
$f(t,\,x):D \subset  \mathbb{R}^{2} \to \mathbb{R}^m$ 이 convex $D$ 에서 정의되어 있으며 어떤 $L>0$ 이 존재하여

$$
\forall (t,\,\boldsymbol{x})\in D,\qquad \left|\dfrac{\partial}{\partial x} f(t, x) \right| \le L
$$

이면 $f$ 는 변수 $x$ 와 Lipschitz 상수 $L$ 에 대해 Lipschitz 조건을 만족한다.

:::
::: {.proof}

$(t,\,x_1),\, (t,\,x_2)\in D$ 일 때 다변수 함수의 평균값 정리에 의해 

$$
f(t,\,x_1) - f(t,\,x_2) = (x_1-x_2)\dfrac{\partial }{\partial x}f(t,\,x)
$$

를 만족하는 $x\in (x_1,\, x_2)$ 가 항상 존재한다. $D$ 가 convex 이므로 $(t,\,x)\in D$ 이다. 따라서 

$$
|f(t,\,x_1) - f(t,\,x_2) | = \left|(x_1-x_2)\dfrac{\partial }{\partial x}f(t,\,x)\right| \le L|x_1-x_2|
$$
...
아래의 정리는 일계 선형 미분방정식의 초기값 문제의 해의 존재성과 유일성을 말한다. 증명은 꽤나 긴데 여기서는 생략한다. 관심이 있으면 Moris Tenenbaum, Harriy Poland 의 *Ordinary Differential Equations* 를 참고하라. 

::: {#thm-lipschitz_condtion}

$D = [a,\,b] \times (-\infty,\,\infty)$ 에서 정의된 함수 $f(t,\,x) :D \to \mathbb{R}$ 이 유계이며 $(t_0,\, x_0) \in D$ 라고 하자. 이 때 $x'(t) = f(t,\,x)$ 와 $x(t_0)= x_0$ 를 만족하는 $x:[a,,\,b]\to \mathbb{R}$ 이 항상 존재하며 유일하다.
...
$$
x(t_{i+1}) =  x(t_i) + f(t_i, x(t_i)) h
$$ {#eq-forward_euler_method}

로 근사한다. 이 때 $t_i,\,i=0,\,1,\ldots$ 를 **mesh point** 라고 한다. 이렇게 $x(t_{i+1}) = x(t_i)+ f(t_i,\,x_i)h$ 로 근사하는 오일러 방법을 **전방 오일러 방법 (forward Euler method)** 혹은 **양함수적 오일러 방법(explicit Euler method)** 라고 한다. 보통 **오일러 방법(Euler method)** 라고 할 때는 전방 오일러 방법을 의미한다. 이에 반해
...
$$
x(t_{i+1}) = x(t_i) + f(t_{i+1},\,x(t_{i+1}))h
...
$$
\boldsymbol{x}(t_{i+1}) = \boldsymbol{x} (t_i) + \boldsymbol{f}(t_i, x_1(x_i), \ldots, x_n(t_i))\, h
$$

를 이용하여 $\boldsymbol{x}(t_{i+1})$ 값을 구한다. 아래와 같이 julia 로 구현 할 수 있다.
