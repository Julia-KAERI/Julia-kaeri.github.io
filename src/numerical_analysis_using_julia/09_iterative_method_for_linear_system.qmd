...
$\boldsymbol{A} \in \mathcal{M}_{n \times n}(\mathbb{F})$ 를 이용한 선형방정식 $\boldsymbol{Ax}=\boldsymbol{b}$ 이 주어졌을 때 $\tilde{\boldsymbol{x}}\in \mathcal{M}_{n}(\mathbb{F})$ 에 대한 **residual vector** $\boldsymbol{r}$ 는 다음과 같이 정의된다.
