...
미분 방정식에서 구하고자 하는 함수 $f$ 와 $f$ 의 모든 도함수의 곱의 차수(degree) 가 모든 방정식의 항에서 같을 때 이를 **homogeneous** 라고 한다. 즉 $(y')^2 + yy' =0$ 은 homogenous 이며 $y' + y + c=0$ 은 $1$ 차항 $y',\,y$ 와 $0$ 차항 $c$ 때문에 inhomogeneous 이다. 또한 $y'' + yy' = 0$ 도 inhomogeneous 이다.
...
@eq-simple_2nd_ode 꼴로 주어진 2차 LHODE 를 생각하자. 이 경우 $y$ 를 $x$ 의 급수로 놓고 해를 구하는 것을 Frobenius 방법이라고 한다. 즉,
...
$x_0$ 를 [regular singularity](A02_ode.qmd#sec-ode_singularity) 로 잡으면 많은 경우 미분방정식의 해를 찾는 데 유용하지만 [irregular siglularity](A02_ode.qmd#sec-ode_singularity) 의 경우는 Frobenius 방법으로 해를 구할 수 없다.
...
2차 LHODE 는 최대 두개의 독립적인 해를 가진다.

:::

::: {.proof}

2차 linear homogeneous ODE 
...
이다. 따라서 2차 linear homogeneous ODE 의 선형 독립인 해는 많아야 2개이다. $\square$
...
2차 linear homogeneous ode 연산자 $\mathcal{L}$ 에 대해
...
위에서 설명했듯이 미분연산자가 hermitian 이라면 매우 좋은 성질을 갖게 된다. 만약 자체로는 hermtian 이 아니지만 약간 변형하여 hermitian 으로 만들 수 있다면? 자 @eq-ode_2nd_lhode 형태의 미분방정식이 hermitian 이 될 조건은 아래의 두가지 이다.
...
$$
\overline{p}_0 = \exp \left(\int^x \dfrac{p_1(t)}{p_0(t)}\, dt\right),\, \overline{p}_1 = \dfrac{p_1(x)}{p_0(x)}\exp \left(\int^x \dfrac{p_1(t)}{p_0(t)}\, dt\right)
$$
에 대해 

$$
\mathcal{L}'\psi (x) = \left[\overline{p}_0 (x) \dfrac{d}{dx^2} + \overline{p}_1(x) \dfrac{d}{dx} + w(x)p_2(x)\right]\psi(x)
$$
...
로 정의하자. 이 때 $w(x)$ 를 weighting function 이라고 한다. 그렇다면 @eq-odd_hermitian_cond_1 은
