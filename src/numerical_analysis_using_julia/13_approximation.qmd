...
@eq-least_square_for_polynomial_linear_equation_matrix-definition 에 의해 정의된 $\boldsymbol{X}$ 는 $(n+1)\times (n+1)$ 대칭행렬이다. $\boldsymbol{X}$ 에 대해 $\boldsymbol{c}^T\boldsymbol{Xc} = 0$ 인 벡터 $\boldsymbol{c}$ 가 존재한다고 하자. 
